...
void Rfm70::initSPI(uint8_t clk_div)
{
...
void Rfm70::initHardware(uint8_t irq)
{
...
void Rfm70::initRegisters(void)
{
	...
	Serial.println("load rfm70 register successful\n");
	#endif
...
void Rfm70::setBegin(uint8_t irq, uint8_t clk_div) 
{
  	initHardware(irq);
  	initSPI(clk_div);
...
uint8_t Rfm70::transmitSPI(uint8_t val) 
{
  SPDR = val;
  while (!(SPSR & _BV(SPIF)))
    ;
...
uint8_t Rfm70::sendPayload(uint8_t * payload, uint8_t len, uint8_t toAck)	// choose 0=nAck, 1=AckRequest
{
...
void Rfm70::debug(uint8_t token)
{
	...
	{
		case 0x01:
		Serial.println("rfm error: the reseive chip id is invalid!\n");
		break;

		case 0x02:
		Serial.println("rfm error: the fifo is full!\n");
		break;
	}
}
#endif

void Rfm70::spiSetClockDivider(uint8_t rate) 
{
