...
static MyMessage msgTmp; // Buffer for temporary messages (acks and nonces among others).
			...
			if (type == I_FIND_PARENT_RESPONSE) {
				...
				if (type == I_REBOOT) {
					...
					if (!mGetAck(msg)) {
						...
						msgTmp = msg;
						mSetRequestAck(msgTmp,false); // Reply without ack flag (otherwise we would end up in an eternal loop)
						mSetAck(msgTmp,true);
						msgTmp.sender = nc.nodeId;
						msgTmp.destination = msg.sender;
						sendRoute(msgTmp);
					}
