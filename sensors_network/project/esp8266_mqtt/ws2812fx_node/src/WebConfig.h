...
namespace WebConfig {
  CfgData* _cfgData = NULL;
  ESP8266WebServer server(HTTP_CFG_SERVER_PORT);

  void begin(CfgData* cfgData) {
    _cfgData = cfgData;
    ...
    server.on("/", []() {
      ...
      index_html.replace("{mys_node_id}", _cfgData->mys_node_id);
      index_html.replace("{mys_node_alias}", _cfgData->mys_node_alias);
      index_html.replace("{mys_node_led_count}", _cfgData->mys_node_led_count);

      index_html.replace("{mqtt_server}", _cfgData->mqtt_server);
      index_html.replace("{mqtt_port}", _cfgData->mqtt_port);
      index_html.replace("{mqtt_user}", _cfgData->mqtt_user);
      index_html.replace("{mqtt_passwd}", _cfgData->mqtt_passwd);
      index_html.replace("{mqtt_in_topic_prefix}", _cfgData->mqtt_in_topic_prefix);
      index_html.replace("{mqtt_out_topic_prefix}", _cfgData->mqtt_out_topic_prefix);
    ...
    server.on("/save", []() {
      ...
      if(server.hasArg("mys_node_id")) {
        strncpy(_cfgData->mys_node_id, server.arg("mys_node_id").c_str(), MYS_NODE_ID_FIELD_MAX_LEN);
      }

      if(server.hasArg("mys_node_alias")) {
        strncpy(_cfgData->mys_node_alias, server.arg("mys_node_alias").c_str(), MYS_NODE_ALIAS_FIELD_MAX_LEN);
      }

      if(server.hasArg("mys_node_led_count")) {
        strncpy(_cfgData->mys_node_led_count, server.arg("mys_node_led_count").c_str(), MYS_NODE_LED_COUNT_FIELD_MAX_LEN);
      }

      if(server.hasArg("mqtt_server")) {
        strncpy(_cfgData->mqtt_server, server.arg("mqtt_server").c_str(), MQTT_SERVER_FIELD_MAX_LEN);
      }

      if(server.hasArg("mqtt_port")) {
        strncpy(_cfgData->mqtt_port, server.arg("mqtt_port").c_str(), MQTT_USER_FIELD_MAX_LEN);
      }

      if(server.hasArg("mqtt_user")) {
        strncpy(_cfgData->mqtt_user, server.arg("mqtt_user").c_str(), MQTT_USER_FIELD_MAX_LEN);
      }

      if(server.hasArg("mqtt_passwd")) {
        strncpy(_cfgData->mqtt_passwd, server.arg("mqtt_passwd").c_str(), MQTT_PASS_FIELD_MAX_LEN);
      }

      if(server.hasArg("mqtt_in_topic_prefix")) {
        strncpy(_cfgData->mqtt_in_topic_prefix, server.arg("mqtt_in_topic_prefix").c_str(), MQTT_IN_TOPIC_PREFIX_FIELD_MAX_LEN);
      }
