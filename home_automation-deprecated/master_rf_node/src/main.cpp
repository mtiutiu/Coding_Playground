...
void Serial_printf(const __FlashStringHelper *format, ...) {
  char buf[PRINTF_BUF];
  va_list ap;
  va_start(ap, format);
#ifdef __AVR__
...
#else
  vsnprintf(buf, sizeof(buf), (const char *)format,
            ap); // for the rest of the world
#endif
  for (char *p = &buf[0]; *p; p++) // emulate cooked mode for newlines
  {
    if (*p == '\n')
...
class RMF12BMasterNode {
  ...
  private:
    ...
    static void invalidCommand() {
      Serial_printf(F(MASTER_NODE_REPLY_MSG_TEMPLATE("%hhu",SERIAL_EOL)),
    ...
    void getNodeUnitTime() {
      Serial_printf(F(MASTER_NODE_REPLY_MSG_TEMPLATE("%lu",SERIAL_EOL)),
        getNodeName(),
        getNodeType(),
        PEER_STATUS_OK,
        now(),
    ...
    static void freeRamCmd() {
      ...
      Serial_printf(F(MASTER_NODE_REPLY_MSG_TEMPLATE("%d",SERIAL_EOL)),
        thisNode->getNodeName(),
        thisNode->getNodeType(),
        PEER_STATUS_OK,
        now(),
    ...
    static void irLearnPeerCmd() {
      ...
      Serial_printf(F(MASTER_NODE_REPLY_MSG_TEMPLATE("{\"channel\":%hhu,\"timeout\":%lu}",SERIAL_EOL)),
        peerIrLearnNameArg,
    ...
    static void listDiscoveredPeersCmd() {
      Serial_printf(F(MASTER_NODE_REPLY_MSG_HEADER("[")),
        thisNode->getNodeName(),
        thisNode->getNodeType(),
        PEER_STATUS_OK,
        now(),
          ...
          lastPeerSlotIndex = thisNode->peersList.getLastIndex(); i < thisNode->peersList.size(); i++) 
      {
        if(thisNode->peersList[i] != NULL) {
          Serial_printf(F("{\"name\":\"%s\",\"type\":%d,\"time\":%lu,\"uptime\":%lu,\"ttl\":%hhu}"),
    ...
    static void getNodeNameCmd() {
      Serial_printf(F(MASTER_NODE_REPLY_MSG_TEMPLATE("\"%s\"",SERIAL_EOL)),
        thisNode->getNodeName(),
        thisNode->getNodeType(),
        PEER_STATUS_OK,
        now(),
