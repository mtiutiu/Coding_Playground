...
void IRsend::sendNEC(uint32_t data, int16_t nbits)
{
  enableIROut(38);
  mark(NEC_HDR_MARK);
  space(NEC_HDR_SPACE);
...
void IRsend::sendSony(uint32_t data, int16_t nbits) {
  enableIROut(40);
  mark(SONY_HDR_MARK);
...
void IRsend::sendRC5(uint32_t data, int16_t nbits)
{
  enableIROut(36);
  data = data << (32 - nbits);
...
void IRsend::sendRC6(uint32_t data, int16_t nbits)
{
  enableIROut(36);
  data = data << (32 - nbits);
  mark(RC6_HDR_MARK);
...
void IRsend::sendSamsung(uint32_t data, int16_t nbits)
{
  enableIROut(38);
...
void IRrecv::blink13(uint8_t blinkflag)
{
...
// As soon as a SPACE gets int32_t, ready is set, state switches to IDLE, timing of SPACE continues.
...
#if defined(NEC_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(SONY_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(RC5_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(RC6_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(PANASONIC_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(JVC_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(SAMSUNG_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(SANYO_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
#if defined(MITSUBISHI_DECODER) || defined(ALL_DECODERS)
#ifdef DEBUG
...
uint8_t IRrecv::decodeNEC(decode_results *results) {
  uint32_t data = 0;
...
uint8_t IRrecv::decodeSony(decode_results *results) {
  uint32_t data = 0;
...
uint8_t IRrecv::decodeRC5(decode_results *results) {
...
uint8_t IRrecv::decodeRC6(decode_results *results) {
...
uint8_t IRrecv::decodePanasonic(decode_results *results) {
    uint64_t data = 0;
    int16_t offset = 1;
    ...
    // decode address
    for (uint8_t i = 0; i < PANASONIC_BITS; i++) {
...
uint8_t IRrecv::decodeJVC(decode_results *results) {
    uint32_t data = 0;
    int16_t offset = 1; // Skip first space
    // Check for repeat
...
uint8_t IRrecv::decodeSamsung(decode_results *results) {
  uint32_t data = 0;
  int16_t offset = 1; // Skip first space
  // Initial mark
...
uint8_t IRrecv::decodeSanyo(decode_results *results) {
  uint32_t data = 0;
...
// Looks like Sony except for timings, 48 chars of data and time/space different
uint8_t IRrecv::decodeMitsubishi(decode_results *results) {
 ...
 */
uint8_t IRrecv::decodeHash(decode_results *results) {
...
void IRsend::sendSharp(uint32_t data, int16_t nbits) {
...
void IRsend::sendDISH(uint32_t data, int16_t nbits)
{
  enableIROut(56);
  mark(DISH_HDR_MARK);
