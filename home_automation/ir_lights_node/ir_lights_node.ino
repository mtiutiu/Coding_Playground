...
const uint8_t IR_LIGHTS_PEER_ID = 3;     //network ID used for this unit
...
class RFM12BIrLightsNode {
  public:
    ...
    inline const PeerType getNodeType() { return IR_LIGHT_TYPE; }
  ...
  private:
    ...
    static const uint32_t IR_KEYPRESS_DELAY_MS = 800;
    static const uint8_t PEER_NAME_MAX_LENGTH = 16;
    ...
    static RFM12BIrLightsNode* thisNode;
    RFM12B radio;
    ...
    LightsScheduler lightsSchedulerInfo;
    IRrecv irrecv;
    uint32_t* irLnKeys;
    boolean irLearnMode;
    ...
    */
    ...
    void startIrLearn(uint32_t timeout) {
      irLearnMode = true;
    ...
    uint32_t irRx() {
      ...
      uint32_t irRecvData = 0;
      decode_results results;

      if(irrecv.decode(&results)) {
        // do not update if the same key was pressed and the allowed delay between key presses hasn't elapsed
        if(((millis() - lastTime) >= IR_KEYPRESS_DELAY_MS) || (results.value != lastKey)) {
          irRecvData = results.value;
        }
    ...
    void irUpdate() {
      uint32_t pressedKey = irRx();

      if(irLearnMode) {
        // if ir learn interval expired exit learn mode immediately
        if(irLearnInterval.isExpired()) {
          stopIrLearn();
          return;
        }
        // if ir learn interval didn't expired and we have a keypress
        if(pressedKey) {
          // if ir learn command is addressed to us
          if(irLearnData.targetPeer == getNodeAddress()) {
            processIrLearnCmd(irLearnData.channel, pressedKey);
          } else {
            // route the detected ir keypress to the target node for learning if it's not for us
            irLearnData.irKey = pressedKey;
            
            payload_t irLearnPayload;
            irLearnPayload.payloadType = IR_LEARN_ROUTE_DATA;
            memcpy(irLearnPayload.payloadData, &irLearnData, sizeof(IrLearnData));
            radio.Send(irLearnData.targetPeer, &irLearnPayload, sizeof(payload_t), false);
          }
          stopIrLearn();
        }
      } else if(pressedKey && !processIrCmd(pressedKey)) {
        // route this ir keypress event through master node to other light nodes if it's not for us
        payload_t irPayload;
        irPayload.payloadType = IR_ROUTE_DATA;
        memcpy(irPayload.payloadData, &pressedKey, sizeof(uint32_t));
        radio.Send(getMasterNodeAddress(), &irPayload, sizeof(payload_t), false);
      }
    }
    ...
    void processIrLearnCmd(uint8_t channel, uint32_t irKeyData) {
      ...
      //   sendReplyToMasterNode(PEER_IR_KEY_ALREADY_ASSIGNED_CODE, PEER_NO_EVENT, PEER_IR_EVENT_SOURCE, "0");
      //   return;
      // }
    ...
    boolean processIrCmd(uint32_t pressedKey) {
      for(uint8_t i = 0; i < getNodeMaxChannels(); i++) {
    ...
    void radioRx() {
      if (radio.ReceiveComplete() && radio.CRCPass()) {
        ...
        switch (recvPayload.payloadType) {
            case IR_LEARN_DATA:
              memcpy(&irLearnData, (IrLearnData*)recvPayload.payloadData, sizeof(IrLearnData));
              stopIrLearn();
              startIrLearn(irLearnData.irLearnTimeout);
              break;
            ...
            case IR_LEARN_ROUTE_DATA:
              ...
              if(((IrLearnData*)recvPayload.payloadData)->targetPeer == getNodeAddress()) {
                // if we were in learning mode before then stop it
                stopIrLearn();
                processIrLearnCmd(
...
RFM12BIrLightsNode rfIrLightsNode(
  MASTER_PEER_ID,
  IR_LIGHTS_PEER_ID,
  NETWORK_ID,
  RF12_868MHZ,
...
RFM12BIrLightsNode* RFM12BIrLightsNode::thisNode;

cmd_func RFM12BIrLightsNode::cmd_actions[] = {
  channelsCountCmd,
...
void setup() {
  rfIrLightsNode.begin();
}

void loop() {
  rfIrLightsNode.update();
}